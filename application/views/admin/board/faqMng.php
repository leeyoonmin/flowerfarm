<div class="wrap board noticeMng ">
  ...
  <div class="divPageTitle">
    <table>
      <tr>
        <td class="engTitle">FAQ Board Manager</td><td class="hanTitle">FAQ 게시판 관리</td><td></td>
      </tr>
    </table>
  </div>
  ...
  <div class="divSearch box width100">
    <table>
      ...
      <tr class="trInput">
        ...
        <td>
          <select class="selectBox CATE">
            <option value="0000">전체</option>
            <?php
              foreach($comboData as $item){
                echo "<option value=\"".$item->CODE."\" "; if(!empty($getData['CATE'])){if($getData['CATE'] == $item->CODE) echo "selected";} echo">".$item->CODE_NM."</option>";
              }
            ?>
          <select>
        </td>
  ...
  <div class="table">
    <div class="tr head">
      ...
      </div><div class="td wd150">글번호</div><div class="td wd150">진행상태</div><div class="td wd500">글제목</div><div class="td wd250">작성일자</div>
    </div>
    <?php
    $rowCnt = 0;
    if(empty($gridData)){
      ...
      foreach($gridData as $row){
        echo "<div class=\"tr body isOneCheck board\">
          ...
          <div class=\"td wd150 TD_WORK_DV\">".$row->ROWNUM."</div><div class=\"td wd150 TD_WORK_DV\">".$row->CATEGORY."</div><div class=\"td wd500 TD_CODE_DV\">".$row->TITLE."</div><div class=\"td wd250 TD_CODE_NM\">".date('Y. m. d',strtotime($row->CREATED))."</div>
        </div>";
        $rowCnt++;
      }
    }
