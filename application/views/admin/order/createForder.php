<div class="wrap forderRequest createForder">

  <div class="divPageTitle">
    <table>
      <tr>
        <td class="engTitle">발주서생성</td><td></td>
      </tr>
    </table>
  </div>
  ...
  <div class="divButton box width100">
    <table>
      <tr>
        <td></td>
        <td>
          <input class="btn blue createForderBtn" type="button" name="" value="발주서생성">
        </td>
        <td>
  ...
  <div class="table">
    <div class="tr head">
      ...
      </div><div class="td wd150">주문일</div><div class="td wd100">배송희망일</div><div class="td wd300">상품명</div><div class="td wd150">주문자</div><div class="td wd100">판매금액</div><div class="td wd100">입금여부</div><div class="td wd100">발주여부</div><div class="td wd100">상세조회</div>
    </div>
    <?php
    $rowCnt = 0;
    if(empty($gridData)){
      ...
      foreach($gridData as $row){
        ...
        echo "<div class=\"tr body isCheck order\">
          ...
          <div class=\"td wd150\">".date('Y-m-d H:i',strtotime($row->ORDER_TIME))."</div><div class=\"td wd150 orderID\" style=\"display:none;\">".$row->ORDER_ID."</div><div class=\"td wd100\">".date('Y-m-d',strtotime($row->DELIVERY_DATE))."</div><div class=\"td wd300\">".$row->PRODUCT."</div><div class=\"td wd150\">".$row->USER_ID;
          if(!empty($row->USER_NICK)) echo "(".$row->USER_NICK.")";
          echo "</div><div class=\"td wd100\">".number_format($row->PAY_PRICE)." 원</div><div class=\"td wd100\">".$row->IS_PAID."</div><div class=\"td wd100\">".$row->IS_FORDER."</div><div class=\"td wd100\"><input class=\"btn orderDetailBtn\" type=\"button\" value=\"상세조회\"></div>
        </div>";
        $rowCnt++;
      }
    }
