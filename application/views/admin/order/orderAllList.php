    ...
    if(empty($gridData)){
      ...
      foreach($gridData as $row){
        echo "<div class=\"tr body isCheck order\">
          ...
          <div class=\"td wd150\">".$row->ORDER_TIME."</div><div class=\"td wd200 orderID\">".$row->ORDER_ID."</div><!--div class=\"td wd200\">".$row->FORDER_ID."</div--><div class=\"td wd200\">".$row->PRODUCT."</div><div class=\"td wd100\">".$row->USER_ID;
          if(!empty($row->USER_NICK)) echo "(".$row->USER_NICK.")";
          echo "</div><div class=\"td wd100\">".number_format($row->PAY_PRICE)." 원</div><div class=\"td wd80\">".$row->IS_PAID."</div><div class=\"td wd100\">".$row->ORDER_STAT_NM."</div><div class=\"td wd100\">".$row->FORDER_STAT_NM."</div>
        </div>";
        $rowCnt++;
      }
    }
