<div class="wrap readyProduct">

  <div class="divPageTitle">
    <table>
      <tr>
        <td class="engTitle">Ready Product</td><td class="hanTitle">상품준비중 관리</td><td></td>
      </tr>
    </table>
  </div>
  ...
  <div class="divButton box width100">
    <table>
      <tr>
        ...
        <td>
          <input class="btn blue onDeliveryBtn" type="button" name="" value="배송중처리">
        </td>
        <td>
  ...
  <div class="table">
    <div class="tr head">
      ...
      </div><div class="td wd100">주문일</div><div class="td wd200">주문번호</div><div class="td wd250">상품명</div>  <div class="td wd150">주문자</div><div class="td wd100">입금금액</div><div class="td wd100">입금여부</div><div class="td wd100">주문상태</div><div class="td wd100">발주상태</div>
    </div>
    <?php
    $rowCnt = 0;
    if(empty($gridData)){
      ...
      foreach($gridData as $row){
        ...
        echo "<div class=\"tr body isCheck order\">
          ...
          <div class=\"td wd100\">".($row->ORDER_TIME)."</div><div class=\"td wd200 orderID\">".$row->ORDER_ID."</div><div class=\"td wd250\">".$row->PRODUCT."</div><div class=\"td wd150\">".$row->USER_ID;
          if(!empty($row->USER_NICK)) echo "(".$row->USER_NICK.")";
          echo "</div><div class=\"td wd100\">".number_format($row->PAY_PRICE)." 원</div><div class=\"td wd100\">".$row->IS_PAID."</div><div class=\"td wd100\">".$row->ORDER_STAT_NM."</div><div class=\"td wd100 forderStat\"><input type=\"hidden\" value=\"".$row->FORDER_STAT."\">".$row->FORDER_STAT_NM."</div>
        </div>";
        $rowCnt++;
      }
    }
